...
class URIExtract(ScannerPlugin):
    ...
    def _run(self, suspect):
        ...
        if suspect.size > maxsize:
            self.logger.info(
                'Not scanning - message too big (message %s  bytes > config %s bytes )' % (suspect.size, maxsize))
            return DUNNO
    ...
    def get_decoded_textparts(self, messagerep):
        ...
        for part in messagerep.walk():
            ...
            if contenttype.startswith('text/') or fname.endswith(".txt") or fname.endswith(".html") or fname.endswith(
                    ".htm"):
...
class URIExtractTest(unittest.TestCase):
    ...
    def test_url_without_file(self):
        ...
        uris = self.candidate.extractor.extracturis(txt)
        self.assertTrue('http://roasty.familyhealingassist.ru?coil&commission' in uris,
                        'did not find uri, result was %s' % uris)
